...
BLEPeripheral::BLEPeripheral(void) :
    ...
    _service_data_uuid(NULL),    /*BHN*/
    _service_data(NULL),         /*BHN*/
    _service_data_length(0),     /*BHN*/
    _local_name(NULL),
    _appearance(0),
    _central(this),
    _attributes(NULL),
...
/*BHN
 * Modified version of _advDataInit()
 * that supports one 16-bit UUID Service Data block,
 * which is needed to support Eddystone URL beacons.
 */
void
    ...
    if (_advertise_service_uuid) {
        ...
        if (BT_UUID16 == uuid.type) {
            ...
            *adv_tmp++ = (1 + sizeof(uint16_t)); /* Segment data length */
            /*BHN  *adv_tmp++ = BLE_ADV_TYPE_INC_16_UUID; */
            *adv_tmp++ = BLE_ADV_TYPE_COMP_16_UUID; // Needed for Eddystone /*BHN*/
